...
export async function fetchHackerNewsTrends(): Promise<TrendingSignal[]> {
  ...
  try {
    ...
    // 取前 30 个分批并行拉详情（每批 5 个，避免触发 Firebase 限速）
    ...
    const HN_SCORE_THRESHOLD = parseInt(process.env.HN_SCORE_THRESHOLD ?? '50', 10);
    for (const item of batchItems) {
      if (!item || item.type !== 'story' || !item.title) continue;
      const title = item.title;
      const score = item.score ?? 0;
      if (score <= HN_SCORE_THRESHOLD) continue;
...
export async function fetchGitHubTrending(): Promise<TrendingSignal[]> {
  ...
  const MIN_STARS = parseInt(process.env.GH_TRENDING_MIN_STARS ?? '100', 10);
  const API = 'https://api.github.com/search/repositories';
  const params = {
    q: `stars:>${MIN_STARS} pushed:>2026-01-01`,
    sort: 'stars',
    order: 'desc',
    per_page: parseInt(process.env.GH_TRENDING_PER_PAGE ?? '30', 10)
  };
...
export async function fetchStackOverflowPainPoints(): Promise<TrendingSignal[]> {
  ...
  const MIN_SCORE = parseInt(process.env.SO_MIN_SCORE ?? '20', 10);
          for (const item of items) {
            const score = item.score ?? 0;
            const answers = item.answer_count ?? 0;
            // 严选：score > MIN_SCORE（广泛用户关注）且 answer_count === 0（完全无人解决）
            if (score <= MIN_SCORE || answers !== 0) continue;
